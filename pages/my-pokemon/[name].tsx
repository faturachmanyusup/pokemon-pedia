...
export default function PokemonDetail(props: PokemonDetailProps): JSX.Element {
  ...
  const [wasReleased, setWasReleased] = useState<boolean>(false);

  const [overallStat, setOverallStat] = useState<number>(0);
      ...
      .then(localPokemon => {
        queryGetPokemon({
          ...
          onCompleted: (res) => {
            ...
            res.pokemon.stats.forEach((stat: { base_stat: number }) => {
...
export const getServerSideProps: GetServerSideProps = async ({ params }: GetServerSidePropsContext) => {
  return { props: { name: params?.name as string } };
};
